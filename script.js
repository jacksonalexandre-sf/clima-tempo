...
document.querySelector('.busca').addEventListener('submit', async (event) => {
    ...
    if(input !== ''){
        ...
        if(json.cod === 200){
            // Chama a função showInfo com um objeto passado no parametro.
            showInfo({
                name: json.name,
...
// Essa função vai mostrar os resultados na tela.
function showInfo(json){
    ...
    document.querySelector('.tempInfo').innerHTML = `${Math.floor(json.temp)} <sup>ºC</sup>`
